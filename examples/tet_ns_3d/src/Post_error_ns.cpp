    ...
    const double &t, double &val_x, double &val_y, double &val_z )
{
  ...
  // ==== Test manu solu ====
  val_z = x*x*x*x + y*y*y*y + 2.0*x*x*y*y - 0.09*x*x - 0.09*y*y;
  // ==== End of test ====
}
    ...
    const double &t, Matrix_3x3 &grad_velo )
{
  ...
  // ==== Test manu solu ====
  grad_velo(2,0) = 4.0*x*x*x + 4.0*x*y*y - 0.18*x; 
  grad_velo(2,1) = 4.0*x*x*y + 4.0*y*y*y - 0.18*y; 
  grad_velo(2,2) = 0.0;
  // ==== End of test ====
}
      ...
      const double &t, double &val_x, double &val_y, double &val_z )
{
  ...
  // ==== Test manu solu ====
  val_z = -mu * (4.0*R_pipe*R_pipe*R_pipe - 0.18*R_pipe); 
  // ==== End of test ====
}
